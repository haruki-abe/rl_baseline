...
if __name__ == "__main__":
    ...
    for i in range(int(args.max_steps)):
        ...
        if i >= args.warmup_steps:
            critic_loss, actor_loss, alpha_loss, alpha = policy.train(replay_buffer,args.batch_size)
        ...
        if done:
            ...
            if i >= args.warmup_steps:
                wandb.log({"train/critic_loss":critic_loss,"actor_loss": actor_loss, "alpha_loss":alpha_loss, "alpha":alpha}, step = i+1)
            state, done = env.reset(), False
            episode_num += 1
            episode_steps = 0
            episode_reward = 0
